...
| An action that survives SIGKILL is reported, not hidden | [#140](https://github.com/lacs-project/sysknife/issues/140) | Actions run in their own process group and the deadline signals the group, so `sudo` forking no longer leaves privileged work running past a timeout that claims to have killed it. If the group still has members after SIGKILL the daemon reports `ActionNotStopped` rather than a plain timeout, and skips the automatic rollback: starting `rpm-ostree rollback` over a transaction that may still be live is worse than leaving it, because doing nothing is recoverable and doing both is not. An operator seeing that error should inspect the host before retrying. |
