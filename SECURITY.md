...
| Attribution counts on a broken chain are claims | — | The census spans every row read while verification stops at the first break, so on a `broken` or `cannot_verify` verdict `rows_censused` exceeds `rows_checked` and the surplus rows are exactly the ones an attacker could have written. The counts are reported with that caveat in words, and `rows_censused` is published so the gap is measurable, but a reader who takes `attributed_rows` from a broken chain as attribution is reading an attacker's claim. |
